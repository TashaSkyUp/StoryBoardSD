...
class Api:
    def __init__(self, app: FastAPI, queue_lock: Lock):
        ...
        self.add_api_route("/sdapi/v1/train/embedding", self.train_embedding, methods=["POST"],
                           response_model=TrainResponse)
        self.add_api_route("/sdapi/v1/train/hypernetwork", self.train_hypernetwork, methods=["POST"],
                           response_model=TrainResponse)
    ...
    def text2imgapi(self, txt2imgreq: StableDiffusionTxt2ImgProcessingAPI):
        ...
        ret = TextToImageResponse(images=b64images, parameters=vars(txt2imgreq), info=processed.js())
        end_time = time.time()
        print(f"Processing text2img request took {end_time - start_time} seconds")
        return ret
    ...
    def extras_batch_images_api(self, req: ExtrasBatchImagesRequest):
        ...
        with self.queue_lock:
            result = postprocessing.run_extras(extras_mode=1, image="", input_dir="", output_dir="", save_output=False,
                                               **reqDict)
    ...
    def train_embedding(self, args: dict):
        try:
            ...
            return TrainResponse(
                info="train embedding complete: filename: {filename} error: {error}".format(filename=filename,
                                                                                            error=error))
