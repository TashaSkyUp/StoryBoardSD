...
class StoryBoardGradio:
    ...
    def get_story_squad_ui(self):
        ...
        with gr.Blocks() as story_squad_interface:
                ...
                with gr.Column(scale=1):
                    with gr.Box():
                        with gr.Column(scale=1):
                            ...
                            if not DEV_MODE: self.all_components["dev_render"].visible = False
                            gr.HTML(value="<span style='padding: 20px 20px 20px 20px;'></span>")

                            self.all_components["benchmark"] = gr.Button('Benchmark',
                                                                  variant='primary')
                            if not DEV_MODE: self.all_components["benchmark"].visible = False
                            gr.HTML(value="<span style='padding: 20px 20px 20px 20px;'></span>")

                            self.all_components["on_preview_audio"] = gr.Button('Preview Audio')
                            gr.HTML(value="<span style='padding: 20px 20px 20px 20px;'></span>")
                            if not DEV_MODE: self.all_components["on_preview_audio"].visible = False
            ...
            if DEV_MODE:
                # GENERATE BUTTON
                self.all_components["submit"].click(lambda: gr.update(visible=True),
                                                    outputs=[robot_dreams])
                # SIMULATE BUTTON
                ...
                # RENDER BUTTON
                if "hide robot dreams as soon as render is pressed":
                    self.all_components["render"].click(fn=lambda: gr.update(visible=False),queue=True,outputs=[top_row])
                    self.all_components["render"].click(fn=lambda: gr.update(visible=False),queue=True, outputs=[storyboard_display])
                    self.all_components["render"].click(fn=lambda: gr.update(visible=False),queue=True, outputs=[robot_dreams])


                if "now actually call the render function":
                    #self.all_components["render"].click(render_main_func_p["fn"],render_main_func_p["inputs"],render_main_func_p["outputs"])
                    self.all_components["render"].click(**render_main_func_kwargs,queue=True)

                if "force hiding of ALL ui elements EXCEPT files_interface after render is complete":
                    render_b =self.all_components["render"]
                    render_b.click(fn=lambda: gr.Tabs.update(selected=0),queue=True, outputs=[file_tabs])
                    render_b.click(fn=lambda: gr.update(visible=False),queue=True, outputs=[robot_dreams])
                    render_b.click(fn=lambda: gr.update(visible=True),queue=True, outputs=[files_interface])

                    for i in range(100):
                        render_b.click(fn=refresh_renders_display,
                                       inputs=gr.State(i),
                                       outputs=videos_comps[i:(i + 1)],
                                       queue=f"videos_{str(int(i/4))}")

                    render_b.click(fn=lambda: gr.update(visible=False),
                                   queue=True, outputs=[render_b])
                # END RENDER BUTTON

                # GENERATE BUTTON
                if "SHOW robot dreams as soon as render is pressed":
                    submit_b = self.all_components["submit"]
                    render_b = self.all_components["render"]
                    submit_b.click(fn=lambda: gr.update(visible=True), outputs=[render_b])
                    submit_b.click(fn=lambda: gr.update(visible=True), outputs=[robot_dreams])
                    submit_b.click(fn=lambda: gr.update(visible=False), outputs=[files_interface])
                    submit_b.click(**generate_main_func_kwargs)
