...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        compare_paths = gr.Checkbox(label='Compare paths (Separate travels from 1st seed to each destination)',
                                    value=False)
        ...
        allowdefsampler = gr.Checkbox(
            label='Allow the default Euler a Sampling method. (Does not produce good results)', value=False)

        return [rnd_seed, seed_count, dest_seed, steps, unsinify, loopback, save_video, video_fps, show_images,
                compare_paths, allowdefsampler]
    ...
    def run(self, p, rnd_seed, seed_count, dest_seed, steps, unsinify, loopback, save_video, video_fps, show_images,
            compare_paths, allowdefsampler):
        initial_info = None
        images = []
        ...
        for s in range(len(seeds)):
            ...
            numsteps = 1 if not loopback and s + 1 == len(seeds) else int(
                steps)  # Number of steps is 1 if we aren't looping at the last seed
            ...
            if save_video and compare_paths and numsteps > 1:
                ...
                clip.write_videofile(os.path.join(travel_path, f"travel-{travel_number:05}-{s:04}.mp4"), verbose=False,
                                     logger=None)

        if save_video and not compare_paths:
            clip = ImageSequenceClip.ImageSequenceClip([np.asarray(t) for t in images], fps=video_fps)
            clip.write_videofile(os.path.join(travel_path, f"travel-{travel_number:05}.mp4"), verbose=False,
                                 logger=None)
